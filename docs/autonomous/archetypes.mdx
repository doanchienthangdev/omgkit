...
graph TD
    ...
    F -->|Yes| G[CLI Tool]
    F -->|No| H[API Service]
    D -->|Web| I{Has payments?}
    D -->|Mobile| J[Mobile App]
    D -->|Desktop| K[Desktop App]
    D -->|Game| L[Game]
    D -->|Simulation| M[Simulation]
    I -->|Yes| N[SaaS MVP]
    I -->|No| O{Uses AI?}
    O -->|Yes| P{Building models?}
    O -->|No| Q[Full-Stack App]
    P -->|Yes| R[AI Model Building]
    P -->|No| S[AI-Powered App]
```
